 ...
 */
export function Toggle({ on = false, onChange, accentHex, offColor, size = 'md', disabled = false, label, style = {} }) {
    ...
    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 10, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.45 : 1, ...style }}>
      <div
        ...
        style={{
          ...
          background: on ? `linear-gradient(135deg, ${accent}, ${accent}cc)` : trackOff,
      ...
      {label && (
        <span style={{ fontFamily: TYPOGRAPHY.sans, fontSize: TYPOGRAPHY.size.base, color: COLORS.textSecondary }}>
          {label}
        </span>
      )}
    </label>
