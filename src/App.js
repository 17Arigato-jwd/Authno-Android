...
/* === EDITOR === */
function Editor({ current, onEditTitle, onEditContent, onToggleMenu, accentHex, goalWords, onStreakUpdate, onToggleSidebar }) {
        ...
        <div className="flex items-center gap-2">
          {mobile && (
            <button
              ...
              className="p-2 border border-white/30 rounded-md hover:bg-white/5 transition"
          ...
          <input
            ...
            className="bg-transparent text-white text-lg font-semibold focus:outline-none border-b border-transparent focus:border-white/20 max-w-[45vw] truncate"
        ...
        {current ? (
          <>
            ...
            <div
              ...
              className="w-full min-h-[400px] bg-[#0f0f10] text-white p-4 rounded-lg shadow-inner focus:outline-none overflow-auto mt-20 leading-relaxed text-base"
...
export default function App() {
  ...
  useEffect(() => {
    const handleRestore = (event) => {
      if (event.data.type === "restored-books") {
        setSessions((prev) => {
          const newOnes = event.data.books.filter((b) => !prev.some((s) => s.filePath === b.filePath));
          return [...newOnes, ...prev];
        });
      }
  ...
  return (
    <div className="authno-root flex text-white relative" style={{ height: '100dvh', overscrollBehavior: 'none' }}>
      <Background
      ...
      <div className="fixed bottom-4 right-4 flex items-center gap-3 text-white/40 text-sm select-none" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {lastSaved && <span className="transition-opacity duration-500 opacity-80">Saved ✓ ({lastSaved})</span>}
        <button
          onClick={() => window.location.reload()}
          className={`p-2 rounded-full border border-white/20 hover:border-white/40 transition ${inactive ? "opacity-70 hover:opacity-100" : "opacity-30"}`}
          title="Reload"
        >
