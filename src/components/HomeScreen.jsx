 ...
 * Returns null if bytes is null/zero (so we can filter it out of meta arrays).
 */
...
// ── Permission banner shown when storage access is denied / not yet granted ───
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-3)', lineHeight: 1.5 }}>
        AuthNo needs permission to search your entire device for <code style={{ color: '#fff' }}>.authbook</code> files —
        similar to how Adobe Acrobat finds PDFs automatically.
      </p>
      <button
        onClick={onRequest}
        style={{
...
export default function HomeScreen({
  ...
  // null = unknown, 'granted' | 'denied' | 'prompt'
  const [storagePermission, setStoragePermission] = useState(null);

  // ── Scan device for .authbook files ──────────────────────────────────────
  ...
  // ── Request full-device storage permission, then scan ─────────────────
  ...
  // ── On mount: check permission status then scan ───────────────────────
  useEffect(() => {
    (async () => {
      try {
        ...
        setStoragePermission(status);
        // Scan regardless — on Android <13 READ_EXTERNAL_STORAGE may already
        // be granted; on Android 13+ we use MediaStore which needs no extra
        // permission for our own files.
        await scanDevice();
      } catch {
        setStoragePermission('denied');
        setLoadingDevice(false);
  ...
  // ── Re-scan when switching to device tab ─────────────────────────────
  useEffect(() => {
      ...
      <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: '20px', flex: 1 }}>
        ...
        <div style={{ ...glassCard, padding: '0', overflow: 'hidden', flex: 1, minHeight: 0 }}>
          ...
          <div style={{
            ...
            {activeTab === 'device' && (
              <>
                {/* Permission banner — only when permission is denied/not granted */}
                {needsPermission && (
                  <PermissionBanner
                    accentHex={accentHex}
                  ...
                  : deviceBooks.length === 0
                    ? (
                      <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-5)', fontSize: '14px' }}>
                        ...
                        {needsPermission && (
                          <span> Grant storage access above to scan all folders.</span>
                        )}
                      </div>
                    )
                    : deviceBooks.map((book, i) => (
                      <BookCard
                        ...
                        title={book.title}
                        meta={[
                          // Folder path — replaces word count on this tab
              ...
              recentBooks.length === 0
                ...
                : recentBooks.map(book => (
                  <BookCard
                    ...
                    title={book.title}
                    meta={[
                      // Fixed label — replaces filesize/filepath
                      '.authbook',
                      timeAgo(book.updated || book.created),
                    ]}
